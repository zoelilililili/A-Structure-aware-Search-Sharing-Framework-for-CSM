...
void ComputeSetIntersection::ComputeCNMergeBasedAVX2(const VertexID* larray, const ui l_count,
    ...
    if (size_ratio > 2) {
        ...
        // AVX2 path loads 8x32-bit lanes from each array via _mm256_loadu_si256.
        // The previous bounds (li+1, ri+3) were insufficient and could read past the end
        // when lc/rc < 8, causing occasional access violations on Windows.
        if (li + 7 < lc && ri + 7 < rc) {
