 ...
 */
// [无向边核对] Edge 是有序对 (first,second)；逻辑上表示无向边时，(a,b) 与 (b,a) 是否等价取决于使用处是否规范化或双向查找。
...
typedef struct Phase3SplitOrders {
    bool enabled_ = false;
    uint32_t block_id_ = std::numeric_limits<uint32_t>::max();
    uint32_t cut_vertex_qv_ = std::numeric_limits<uint32_t>::max();
    std::vector<char> block_inner_mask_;
    OrdersPerEdge main_orders_;
    OrdersPerEdge block_orders_;
} Phase3SplitOrders;
