...
#include <cassert>
bool GlobalViewManager::nlf_check_sparse(const std::vector<std::pair<uint32_t, uint32_t>> &query_u_nlf,
                                         uint32_t data_v) const {
    const auto &v_nlf = data_nlf_[data_v];
    for (const auto &kv : query_u_nlf) {
        if (v_nlf[kv.first] < kv.second)
            return false;
    }
    return true;
}
void GlobalViewManager::build_subgraph_vertex_nlf(const Graph *sub_query,
                                                  std::vector<std::vector<std::pair<uint32_t, uint32_t>>> &out) const {
    if (!sub_query) {
        out.clear();
        return;
    }
    const uint32_t n = sub_query->getVerticesCount();
    out.assign(n, {});
    for (uint32_t u = 0; u < n; ++u) {
        uint32_t u_nbrs_count;
        auto u_nbrs = sub_query->getVertexNeighbors(u, u_nbrs_count);
        spp::sparse_hash_map<std::pair<LabelID, LabelID>, uint32_t> frequency;
        for (uint32_t i = 0; i < u_nbrs_count; ++i) {
            uint32_t uu = u_nbrs[i];
            std::pair<LabelID, LabelID> key = {sub_query->getEdgeLabelByVertex(u, uu), sub_query->getVertexLabel(uu)};
            const auto it = nlf_mapping_.find(key);
            assert(it != nlf_mapping_.end());
            frequency[key] += 1;
        }
        for (const auto &kv : frequency) {
            const auto mit = nlf_mapping_.find(kv.first);
            assert(mit != nlf_mapping_.end());
            out[u].emplace_back(mit->second, kv.second);
        }
    }
}
