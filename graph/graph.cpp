...
void Graph::loadGraphFromFile(const std::string &file_path) { 
    ...
    infile.close();
    labels_count_ = (ui)labels_frequency_.size() > (max_label_id + 1) ? (ui)labels_frequency_.size() : max_label_id + 1; // 标签空间上界
...
void Graph::loadGraphFromFileCompressed(const std::string &degree_path, const std::string &edge_path, // 从四个二进制文件加载压缩图
    ...
    deg_file.read(reinterpret_cast<char *>(degrees), sizeof(int) * vertices_count_); // 读入所有度数
    ...
    for (ui i = 0; i < vertices_count_; ++i) { 
        if (degrees[i] > 0) { 
            ...
            adj_file.read(reinterpret_cast<char *>(neighbors_ + offsets_[i]), degrees[i] * sizeof(int)); // 连续读入该点所有邻居
        }
    }
    ...
    label_file.read(reinterpret_cast<char *>(vertex_labels_), sizeof(int) * vertices_count_); // 一次性读入
...
void Graph::storeCompressedGraph(const std::string &degree_path, const std::string &edge_path, // 将当前 CSR 图写出为四个二进制文件
    ...
    edge_outputfile.write(reinterpret_cast<const char *>(neighbors_), edge_array_bytes); // 整块写出
    ...
    if (edge_labels_ != nullptr && !edge_label_file_path.empty()) {
        ...
        edge_label_outputfile.write(reinterpret_cast<const char *>(edge_labels_), edge_array_bytes); // 与 neighbors 对齐
...
void Graph::loadGraphFromFileWithoutMeta(const std::string &file_path) { 
    ...
    labels_count_ =
            (ui) labels_frequency_.size() > (max_label_id + 2) ? (ui) labels_frequency_.size() : max_label_id + 2; // 标签表大小（含未出现顶点的占位标签）
